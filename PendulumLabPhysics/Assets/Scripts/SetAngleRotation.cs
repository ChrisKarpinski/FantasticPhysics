...
public class SetAngleRotation : MonoBehaviour {
	...
	void Update () {
        ...
        Debug.Log(GameObject.Find("string").GetComponent<Rigidbody2D>().velocity.magnitude/100);
        ...
        {
            kineticEnergy = 0.5f * GameObject.Find("Mass Slider").GetComponent<Slider>().value * savedVelocity.sqrMagnitude / (124^2);
            
        }
        
        else
        {
            kineticEnergy = 0.5f * GameObject.Find("Mass Slider").GetComponent<Slider>().value * GameObject.Find("string").GetComponent<Rigidbody2D>()
                .velocity.sqrMagnitude / (124^2);
        }
        */
        if (timeValue == 0)
        {
        ...
        if (timeValue > 0)
        {
            potentialEnergy = GameObject.Find("Mass Slider").GetComponent<Slider>().value * Physics2D.gravity.magnitude / 124 * (GameObject.Find("pendulum").GetComponent<RectTransform>
                ().position.y - originalHeight) / 124;


            if (timeValue < 0.05)
            {
                mechanicalEnergy = potentialEnergy;
            }
            kineticEnergy = mechanicalEnergy - potentialEnergy;
        }

        if (GameObject.Find("string").GetComponentInParent<Rigidbody2D>().velocity.magnitude / 100 < 0.1 && period == 0
            && GameObject.Find("string").GetComponent<RectTransform>().rotation.z > 0 && timeValue > 1
            && GameObject.Find("Demo Text").GetComponent<Text>().text != "Continue")
        {
            // record the period
            period = timeValue;
        }
    ...
    public void SetGravity ()
    {
        ...
        Physics2D.gravity = new Vector2(0, -gravitySlider*124);
